...
- **Deep Verification** — a Manage-page card (and `GET /admin/verify`) that proves the database is intact with exact counts and zero sampling: page counts and numbering, duplicates and orphans, every page image on disk, text consistency, embedding presence at exact dimensions, visual-embedding blob byte-integrity, chunk existence and completeness, extraction coverage (including the confirmed-empty protocol), TOC-summary coverage, vector-index dimension configuration, and index health. The verdict is PASS only at literally zero violations.
...
| **Hybrid** | Strategies: `rrf` (BM25 + dense + bge-reranker, default), `graph_boosted`, `vector_first`, `graph_first`, `community`. Graph strategies use stopword filtering, a Lucene fulltext index on entity names, and a trigram-indexed fuzzy entity matcher (full coverage of 276K+ entity name-forms in ~40 ms). | Tuned search behaviour per query type |
| **Summary** | RAPTOR-by-TOC zoom-out: hierarchical section summaries (leaf section → chapter → whole document) built from the Docling heading structure by the local LLM, searched at any abstraction level. Flat numbered headings gain a synthesized chapter layer; scanned books without headings fall back to page-window sections. | Zoom-out questions: *"what does this book cover"*, *"which sections treat radar odometry"* |
